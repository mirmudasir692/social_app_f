...
import { access_token, user_id } from "../../features/auth/authSlice";
// import { initializeSocket } from "../../api/socket/socketio";
        ...
        <div class="flex">
          <div class="w-12 h-12 mr-4 relative flex flex-shrink-0">
            <img
              ...
              src={`${baseUrl}${receiver.profile_pic}`}
              alt=""
            />
          </div>
          <div class="text-sm">
            <p class="font-bold">{receiver && receiver.username}</p>
            <p>Active 1h ago</p>
          </div>
        </div>
            ...
            {chat.sender.id !== sender_id ? (
              <div class="flex flex-row justify-start">
                <div class="w-8 h-8 relative flex flex-shrink-0 mr-4">
                  <img
                    class="shadow-md rounded-full w-full h-full object-cover"
                    src={`${baseUrl}${chat.sender.profile_pic}`}
                    alt=""
                  />
                </div>
                <div class="messages text-sm text-gray-700 grid grid-flow-row gap-2">
                  <div class="flex items-center group">
                    {chat.message && (
                      <p class="px-6 py-3 rounded-t-full rounded-r-full bg-gray-800 max-w-fit lg:max-w-md text-gray-200">
                        {chat.message}
                      </p>
                    )}
                    <button
...
C15.786,7.8,14.8,8.785,14.8,10s0.986,2.2,2.201,2.2S19.2,11.215,19.2,10S18.216,7.8,17.001,7.8z"
              ...
              <div class="flex flex-row justify-end">
                <div class="messages text-sm text-white grid grid-flow-row gap-2">
                  <div class="flex items-center flex-row-reverse group mt-2">
